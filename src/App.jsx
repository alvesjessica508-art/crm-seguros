...
function AlertaCard({ c, onPreview }) {
  ...
  return (
    <div style={{ background: status.bg, border: `1px solid ${status.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
      <div style={{ flex: 1, minWidth: 180 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
          <span style={{ fontWeight: 700, fontSize: 14, color: "#1a202c" }}>{c.cliente}</span>
          <span style={{ fontSize: 11, background: status.color, color: "#fff", borderRadius: 6, padding: "2px 7px", fontWeight: 700 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
        <div style={{ fontSize: 13, fontWeight: 700 }}>R$ {Number(c.premio).toLocaleString("pt-BR")}</div>
...
export default function App() {
  ...
  const ncVazio = { cliente: "", cpf: "", tipo: "Automóvel", seguradora: "", apolice: "", inicio: "", vencimento: "", premio: "", contato: "" };
  const nlVazio = { nome: "", contato: "", interesse: "Automóvel", origem: "Indicação", status: "Novo", observacao: "" };
  const [nc, setNC] = useState(ncVazio);
  const [nl, setNL] = useState(nlVazio);
  ...
  async function carregarDados() {
    try {
      ...
      const [cls, lds] = await Promise.all([
        sbFetch("clientes", { params: "?order=created_at.desc" }),
        sbFetch("leads",    { params: "?order=created_at.desc" }),
      ]);
      setClientes(cls);
      setLeads(lds);
    } catch {
  ...
  const alertas = useMemo(() =>
    dados.filter(c => c.status.label !== "Vigente").sort((a, b) => a.dias - b.dias),
    [dados]
  );
  ...
  async function salvarNovo() {
    ...
    try {
      await sbFetch("clientes", {
        method: "POST",
        body: { ...nc, premio: parseFloat(nc.premio) || 0, inicio: nc.inicio || null },
      });
      setNC(ncVazio);
      setShowForm(false);
      await carregarDados();
    } catch { setErro("Erro ao salvar cliente."); }
    setSalvando(false);
  }
  ...
  async function excluirCliente(id) {
    ...
    try {
      await sbFetch("clientes", { method: "DELETE", params: `?id=eq.${id}` });
  ...
  async function salvarLead() {
    ...
    try {
      if (editLead) {
        ...
        await sbFetch("leads", { method: "POST", body: { ...nl, data: new Date().toLocaleDateString("pt-BR") } });
      }
      setNL(nlVazio);
  ...
  async function excluirLead(id) {
    ...
    try {
      await sbFetch("leads", { method: "DELETE", params: `?id=eq.${id}` });
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#f1f5f9", gap: 16 }}>
      ...
      <div style={{ color: "#718096", fontSize: 14 }}>Conectando ao banco de dados...</div>
    ...
    <div style={{ fontFamily: "'Segoe UI',system-ui,sans-serif", background: "#f1f5f9", minHeight: "100vh" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1a3a5c 0%,#2e6da4 100%)", padding: "18px 16px 0", color: "#fff" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
          <div>
            ...
            <div style={{ fontSize: 11, opacity: 0.7, marginTop: 1 }}>{new Date().toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}</div>
          </div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button onClick={carregarDados} title="Atualizar" style={{ background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, padding: "7px 11px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>🔄</button>
            {alertas.length > 0 && (
            ...
            <button onClick={() => { setShowLeadForm(true); setEditLead(null); setNL(nlVazio); }} style={{ background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, padding: "7px 11px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              + Lead
            </button>
            <button onClick={() => { setShowForm(true); setNC(ncVazio); }} style={{ background: "#fff", color: "#1a3a5c", border: "none", borderRadius: 8, padding: "7px 11px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              + Apólice
            </button>
          </div>
        </div>
      ...
      <div style={{ padding: 14 }}>
        ...
        {tab === "dashboard" && <>
          ...
          {dados.length === 0 && leads.length === 0 && (
            <div style={{ background: "#fff", borderRadius: 12, padding: 40, textAlign: "center", border: "1px solid #e2e8f0", marginBottom: 14 }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 16, color: "#1a3a5c", marginBottom: 6 }}>Bem-vindo ao CRM Seguros!</div>
              <div style={{ fontSize: 13, color: "#718096", marginBottom: 20 }}>Comece adicionando sua primeira apólice ou lead.</div>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
                <button onClick={() => { setShowForm(true); setNC(ncVazio); }} style={{ background: "#1a3a5c", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Adicionar Apólice</button>
                <button onClick={() => { setShowLeadForm(true); setEditLead(null); setNL(nlVazio); }} style={{ background: "#553c9a", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Adicionar Lead</button>
              </div>
            </div>
          )}
              ...
              {alertas.slice(0, 3).map(c => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid #f1f5f9", gap: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#718096" }}>{c.tipo}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {TIPOS.slice(1).map(tipo => {
                  ...
                  return (
                    <div key={tipo} style={{ flex: 1, minWidth: 85, background: "#f8fafc", borderRadius: 8, padding: "8px 10px", border: "1px solid #e2e8f0" }}>
        ...
        {tab === "alertas" && <>
          <div style={{ background: "#f0fff4", border: "1px solid #9ae6b4", borderRadius: 12, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "#4a5568", display: "flex", alignItems: "center", gap: 8 }}>
            <WaIcon size={16} /> Clique em <b>"Enviar WhatsApp"</b> para abrir o WhatsApp Web com a mensagem pronta.
          </div>
        ...
        {tab === "leads" && <>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => { setShowLeadForm(true); setEditLead(null); setNL(nlVazio); }} style={{ background: "#553c9a", color: "#fff", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              + Novo Lead
            </button>
          </div>
          <div style={{ display: "flex", gap: 7, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
            {STATUS_LEAD.map(s => {
              ...
              const st = SL_STYLE[s];
              return (
                <div key={s} style={{ background: st.bg, border: `1px solid ${st.border}`, borderRadius: 10, padding: "8px 12px", minWidth: 95, textAlign: "center", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {leadsFiltrados.map(l => {
              const st = SL_STYLE[l.status] || SL_STYLE["Novo"];
              return (
                <div key={l.id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      <button onClick={() => abrirPreviewLead(l)} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: WA, color: "#fff", border: "none", borderRadius: 7, padding: "6px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                        <WaIcon size={12} /> Contatar
                      </button>
                      <button onClick={() => abrirEditLead(l)} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 7, padding: "5px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", color: "#4a5568" }}>✏️ Editar</button>
                      {l.status !== "Convertido" && (
                        <button onClick={() => converterLead(l)} style={{ background: "#f0fff4", border: "1px solid #68d391", borderRadius: 7, padding: "5px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", color: "#276749" }}>✅ Converter</button>
                      )}
        ...
        {tab === "lista" && <>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
            <input placeholder="🔍 Buscar..." value={busca} onChange={e => setBusca(e.target.value)} style={{ ...inp, width: 170, flex: "none" }} />
            <select value={filtroStatus} onChange={e => setFS(e.target.value)} style={{ ...inp, width: 155, flex: "none" }}>{STATUS_APOLICE.map(s => <option key={s}>{s}</option>)}</select>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#718096" }}>
                      📅 Vence: {fmt(c.vencimento)} · <span style={{ color: c.status.color, fontWeight: 600 }}>{c.dias < 0 ? `Venceu há ${Math.abs(c.dias)}d` : `Faltam ${c.dias}d`}</span> · 💰 R$ {Number(c.premio).toLocaleString("pt-BR")}
                    </div>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 998, padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 22, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 16, color: "#1a3a5c" }}>🛡️ Nova Apólice</div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }}>
              ...
              <button onClick={salvarNovo} disabled={salvando} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#1a3a5c", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 13, opacity: salvando ? 0.7 : 1 }}>
      ...
      {showLeadForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 998, padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 22, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 16, color: "#1a3a5c" }}>🎯 {editLead ? "Editar Lead" : "Novo Lead"}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ gridColumn: "span 2" }}>
